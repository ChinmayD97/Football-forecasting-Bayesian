...
```{r}
data_all_goalcard = read.csv("C:/Decision Sciences/Term III/IS Term III/Data/EPL Data/all_incidents.csv",header = TRUE)
...
#Data_all contains filtered goals and card outcomes

```{r}
data_y = read.csv("C:/Decision Sciences/Term III/IS Term III/Data/EPL Data/matchdetails.csv",header = TRUE)
```

```{r}
data_x = read.csv("C:/Decision Sciences/Term III/IS Term III/Data/EPL Data/player_details.csv",header=TRUE)
```
...
data1a has data sorted st for each type and game home team observations come first
...
S13 is data with outcome variable (ordinal) and team id variable removed. (S13 - Outcome) will be the design matrix X.
-Yet to add strength of team as a covariate. (Done)
-Yet to match Home/Away team (Done)
...
S14_b does not have duplicate rows or columns.

```{r}
write.csv(s13,"C:\\Decision Sciences\\Term III\\IS Term III\\Data\\EPL Data\\Finaldata4c_complete.csv", row.names = TRUE)
```
